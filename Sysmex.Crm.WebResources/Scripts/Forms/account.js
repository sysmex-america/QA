...
(function (global) {
	...
	global.Sysmex.Forms.account = (function () {
		var formContext;
		...
		function onLoad(executionContext) {
            ...
            setDefaultCountry();
            // We need to add a pre-search event to the partner contact lookup that filters which contacts we can choose
            if (formContext.getControl("smx_partnercontact") != null) {
                formContext.getControl("smx_partnercontact").addPreSearch(addPartnerContactLookupFilter);
            }
            if (formContext.getControl("smx_address") != null) {
                _lockAddressFieldBasedOnSap();
            }
        }

        function _lockAddressFieldBasedOnSap() {
            var addControl = formContext.getControl('smx_address'),
                street1Control = formContext.getControl('address1_line1'),
                street2Control = formContext.getControl('address1_line2'),
                zipControl = formContext.getControl('address1_postalcode'),
                cityControl = formContext.getControl('address1_city'),
                countryControl = formContext.getControl('smx_countrysap'),
                stateControl = formContext.getControl('smx_stateprovincesap');
        ...
        function addPartnerContactLookupFilter() {
            // We want to limit the contacts that are able to be selected in the Partner Contact field
            // to do that, we'll add a custom fetch filter to the Partner Contact field with the Partner Account Id
            var account = formContext.getAttribute("smx_partneraccount").getValue();
            ...
            formContext.getControl("smx_partnercontact").addCustomFilter(fetchXml);
        }
